...
```{r}
  ...
  # SPLIT BY STATE: THIS IS QUITE RESOURCE INTENSIVE - MIGHT BE BEST NOT TO 'FILTER' ON STATE NAME.
  outcome_by_state<-split(outcomes, outcomes$State)
  #PUL OUT THE SELECTED STATE DATA AS A DATA FRAME
  state_df<-outcome_by_state[[state]]
  ...
  head(order_by_MR)
    ...
    rank_it<-rank(order_by_MR[,cond],na.last = TRUE, ties.method = "first")
  ## this must be easier to do
    order_by_MR<-cbind(order_by_MR,rank_it)
  
    class(conditions[,2])
    
    hello<-order_by_MR[,conditions[,2]]
    
    easy_to_read <- cbind(order_by_MR[,2], order_by_MR[,7], order_by_MR[,11], order_by_MR[,17], order_by_MR[,23], order_by_MR[,47])
  head(easy_to_read)
